...
Connect Posessed Browser to n8n to build automated workflows. Your n8n AI agents can control the browser—open pages, click buttons, extract data, and fill forms.
...
Before you start:
- Posessed Browser running with MCP server enabled (Settings → Posessed Browser as MCP)
...
### 4. Connect to Posessed Browser

Click on the MCP Client node and configure:

- **Endpoint**: Your Posessed Browser MCP URL (find it in Settings → Posessed Browser as MCP)
- **Server Transport**: `HTTP Streamable`

<img src="/images/n8n-step5-mcp-client.png" alt="MCP Client configuration with Posessed Browser URL" />
...
```
open google.com in Posessed Browser
```
...
- Make sure Posessed Browser is running
- Check that MCP server is enabled in Posessed Browser settings
- Verify the port number matches your Posessed Browser MCP URL
