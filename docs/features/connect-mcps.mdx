...
description: "Link your apps to Posessed Browser so the assistant can read data and take actions"
---

Connect your favorite apps to Posessed Browser so the assistant can read your emails, check your calendar, post to Slack, update Notion, and more — all through natural conversation.
...
Posessed Browser Connected Apps use the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/), an open standard for connecting AI assistants to external systems. Think of it as a single, consistent way to plug your apps into the assistant.
...
3. Sign in and authorize Posessed Browser
...
A browser window will open to sign in. Once authenticated, add `http://localhost:8000/sse` as a custom MCP in Posessed Browser.

<Note>
Keep the terminal running while you use Posessed Browser. The local server handles authentication and proxies requests to the remote MCP server.
</Note>
...
<Columns cols={3}>
  <Card title="Your data stays local" icon="lock">
    Posessed Browser connects directly to your accounts. Credentials are stored locally on your machine.
  </Card>
  ...
  <Card title="Secure OAuth" icon="shield-check">
    Built-in apps use OAuth flows — Posessed Browser never sees your passwords.
  </Card>
</Columns>
